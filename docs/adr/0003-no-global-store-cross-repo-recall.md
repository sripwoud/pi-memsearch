...
- **User-global store layer** (as [#24](https://github.com/sripwoud/pi-memsearch/issues/24) proposed): rejected. Requires pi-only collection naming and capture routing no other mesh agent knows (fractures ADR 0001 parity), introduces cross-repo capture pollution, and duplicates what `MEMSEARCH_DIR` already does. Upstream's own two-scope design ([memsearch PR #525](https://github.com/zilliztech/memsearch/pull/525), blended multi-scope search) is unmerged and stale since 2026-07 — shipping ahead of it risks reconciling divergent scope semantics later.
